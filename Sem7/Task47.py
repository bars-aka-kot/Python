...
У вас есть код, который вы не можете менять (так часто бывает, когда код в глубине программы используется множество раз и вы не хотите
ничего сломать): 
...
'''Вывод: 
ok'''
transformation = list(map(lambda x: x, values))
...
# values = [int(input("Введите элемент: ")) for i in range(
